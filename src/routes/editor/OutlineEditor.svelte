	...
	>
		<div class="flex max-h-[80vh] w-full max-w-lg flex-col rounded-xl bg-white p-5 shadow-xl">
			<div class="mb-3 flex items-center justify-between">
				<h2 class="text-base font-semibold text-slate-900">Outline / bookmarks</h2>
				<button
					type="button"
			...
			<div class="flex-1 overflow-y-auto">
				{#if editor.outline.length === 0}
					<p class="py-6 text-center text-sm text-slate-500">No bookmarks yet.</p>
				{:else}
						...
						{#each editor.outline as item, i (i)}
							<li class="rounded border border-slate-200 p-2">
								<div class="flex items-center gap-2">
									...
									/>
									<select
										class="rounded border border-slate-300 px-1.5 py-1 text-sm"
									...
									<button
										type="button"
										class="rounded px-1.5 py-1 text-xs text-slate-500 hover:bg-slate-100 disabled:opacity-30"
										title="Move up"
										disabled={i === 0}
								...
								{#if item.children && item.children.length > 0}
									<ul class="mt-2 ml-5 flex flex-col gap-1 border-l border-slate-200 pl-2">
										{#each item.children as child, ci (ci)}
											<li class="flex items-center gap-2">
												<input
													type="text"
													class="flex-1 rounded border border-slate-300 px-2 py-0.5 text-sm"
													value={child.title}
													oninput={(e) =>
